...
async function carregarDados(){
    const retorno = await fetch("../php/tecnico_get.php");
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
...
for(var i=0; i<registros.length; i++){
    ...
    html += `<tr>
                ...
                <td>${objeto.genero}</td>
                <td>
                    <a class="btn btn-primary btn-sm me-2" href='tecnico_alterar.html?id=${objeto.id}'>Alterar</a>
...
async function excluir(id){
    const retorno = await fetch("../php/tecnico_excluir.php?id="+id);
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
