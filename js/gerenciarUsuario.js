...
document.getElementById("novo").addEventListener("click", () => {
    window.location.href = 'usuarioNovo.html';
});
...
async function carregarDados(){
    const retorno = await fetch("../php/usuarioGet.php");
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
...
for(var i=0; i<registros.length; i++){
    ...
    html += `<tr>
                ...
                <td>${objeto.role}</td>
                <td>
                    <a class="btn btn-primary btn-sm me-2" href='usuarioAlterar.html?id=${objeto.id}'>Alterar</a>
...
async function excluir(id){
    const retorno = await fetch("../php/usuarioExcluir.php?id="+id);
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
